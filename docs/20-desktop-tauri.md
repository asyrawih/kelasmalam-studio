...
| Discovery SoundCloud (`/v1/search`, stream) | `fetch` ke `soundcloud.kelasmalam.app` | command `soundcloud_json`/`soundcloud_bytes` dijawab **di dalam proses** oleh pustaka `soundclaude` (dependensi git, `crates/desktop-host/src/soundcloud.rs`) — desktop tidak memakai server discovery sama sekali |
