 ...
 *
 * Paket ini tidak punya `createWebHost()` (itu milik app, docs/25 §1d), jadi
 * host web di sini pun palsu: yang diuji adalah perilaku hook terhadap
 * KONTRAK (`kind`, ada/tidaknya `openAudioFiles`/`onFilesDropped`), bukan
 * implementasi web-nya — itu diuji di `apps/web/src/platform/web.test.ts`.
 */
...
import { getPlatformHost, registerPlatformHostResolver, setPlatformHostForTests } from './index';
...
/** Host web minimal: tanpa dialog native dan tanpa drop OS, seperti web sungguhan. */
function fakeWebHost(): PlatformHost {
  return {
    kind: 'web',
...
beforeEach(() => {
  registerPlatformHostResolver(fakeWebHost);
});
...
describe('getPlatformHost', () => {
  it('memakai resolver yang terdaftar, dan host tes menggantikannya', () => {
  ...
  it('resolver dipanggil sekali (malas) dan dilupakan saat resolver baru didaftarkan', () => {
    const resolver = vi.fn(fakeWebHost);
    registerPlatformHostResolver(resolver);
    expect(resolver).not.toHaveBeenCalled();
    const first = getPlatformHost();
    expect(getPlatformHost()).toBe(first);
    expect(resolver).toHaveBeenCalledOnce();

    const next = fakeDesktopHost();
    registerPlatformHostResolver(() => next);
    expect(getPlatformHost()).toBe(next);
  });

  it('tanpa resolver: galat yang menyebut siapa yang harus mendaftar', async () => {
    // Paket ini tidak boleh diam-diam memilih web: app yang lupa mendaftar
    // harus tahu di baris pertama, bukan lewat dialog simpan yang tidak muncul.
    // Modul dimuat ulang supaya state registry-nya benar-benar kosong.
    vi.resetModules();
    const fresh = await import('./host-registry');
    expect(() => fresh.getPlatformHost()).toThrow(/registerPlatformHostResolver/);
  });
});
