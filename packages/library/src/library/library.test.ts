...
import { HASH, libraryApiContract, track, type Backend, type Step } from './api-contract';
import { formatBytes, formatDuration, summarize } from './model';
...
/*
 * Suite KONTRAK (`api-contract.ts`): satu daftar janji `LibraryApi`, dijalankan
 * di sini untuk klien Worker dan di `apps/desktop/src/library-local/
 * local-api-contract.test.ts` untuk kepustakaan lokal — implementasi desktop
 * tidak lagi ada di bundel web (docs/25 P2), tapi janjinya tetap satu.
 */
libraryApiContract(httpBackend());
