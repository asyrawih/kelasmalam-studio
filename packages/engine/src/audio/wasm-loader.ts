 ...
 * dilacak git (`.gitignore` memuat `/packages/engine/src/wasm/`), jadi `git pull` tidak
...
export function assertArtifactSurface(glue: unknown): void {
  ...
  throw new Error(
    `Artefak WASM lebih lama daripada kodenya: ${missing.length} nama yang dibutuhkan ` +
      `tidak ada di glue (${missing.join(', ')}). Artefak di \`packages/engine/src/wasm/\` tidak ` +
    ...
    if (/incompatible|imported Memory|memory import/i.test(msg)) {
      throw new Error(
        `Artefak WASM tidak cocok dengan engine: ${msg}. Hampir selalu ini berarti ` +
          'artefak di `packages/engine/src/wasm/` lebih lama daripada kodenya — artefak itu ' +
...
async function doLoad(): Promise<LoadedWasm> {
  ...
  //
  // Kalau `packages/engine/src/wasm/` masih kosong, dev-server Vite menjawab request ke
  ...
  // `/packages/engine/src/wasm/`), jadi `git pull` tidak pernah membawanya: sesudah plafon
