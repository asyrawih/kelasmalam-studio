...
// isTauri true, tapi tidak ada satu command pun yang jalan. Dua tes ini
...
/// Isi `const COMMANDS: &[&str] = &[ "a", "b", ... ];` di build.rs.
fn build_rs_commands(src: &str) -> BTreeSet<String> {
    let start = src
        .find("const COMMANDS")
        .expect("COMMANDS ada di build.rs");
    // Kurung siku PERTAMA sesudah `const COMMANDS` milik tipenya (`&[&str]`);
    // isinya dimulai dari `= &[`.
    let open = start + src[start..].find("= &[").unwrap() + "= &".len();
    let close = open + src[open..].find(']').unwrap();
    src[open + 1..close]
        .split(',')
...
#[test]
fn build_rs_generates_permissions_for_every_registered_command() {
    let handlers = rust_handler_names(&commands_rs());
    let generated = build_rs_commands(&build_rs());
    let missing: Vec<_> = handlers.difference(&generated).collect();
    let stale: Vec<_> = generated.difference(&handlers).collect();
    assert!(
        missing.is_empty(),
        "command tanpa izin autogenerated (tambahkan ke COMMANDS di build.rs): {missing:?}"
    );
    assert!(
        stale.is_empty(),
        "COMMANDS di build.rs menyebut command yang tidak terdaftar: {stale:?}"
    );
}

#[test]
fn capability_allows_every_command_for_the_loopback_origin() {
    let handlers = rust_handler_names(&commands_rs());
    ...
    assert!(
        urls.iter()
            .any(|u| u.as_str() == Some("http://127.0.0.1:*")),
        "remote.urls harus memuat origin server loopback"
    );
}
