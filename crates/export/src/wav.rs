...
fn header_bytes(spec: &WavSpec, data_len: u32) -> [u8; 44] {
    let mut h = [0u8; 44];
...
impl WavStreamWriter {
    ...
    /// Header awal dengan ukuran placeholder (0). Dikirim duluan ke Blob parts.
    pub fn placeholder_header(&self) -> [u8; 44] {
        header_bytes(&self.spec, 0)
    }

    /// Header final — dipanggil setelah semua data ditulis dan panjang total
    /// diketahui. Main thread mengganti part pertama dengan ini.
    pub fn patch_header(&self) -> [u8; 44] {
        header_bytes(&self.spec, self.data_bytes.min(u32::MAX as u64) as u32)
    }
...
pub fn encode_all(planar: &[&[f32]], spec: WavSpec, dither: DitherSettings) -> Vec<u8> {
    ...
    out.extend_from_slice(&w.patch_header());
